...
object ListAllExtensions:
  ...
  def findAllExtensions(path: String): Future[Either[String, String]] =
    ...
    Using(Files.walk(Paths.get(path))) {
      ...
      case Failure(error) => error match
        case _: NoSuchFileException => promise.success(Left(s"""Cannot find the "$path" directory."""))
        case exception: Exception => promise.success(Left(exception.getMessage))
